
function Features() {
  return (
    <>
      <div className="relative features flex  mt-40  lg:flex-row flex-col">
        <div
          // data-aos="fade-left"
          className="flex flex-col  mt-16  items-center lg:justify-start justify-center lg:w-1/4 hover:scale-105 hover:shadow-lightBlueShadow rounded-3xl transition-all p-5">
          <svg
            className="h-20 m-4"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512 512"
          >
            <path
              fill="white"
              d="M156.6 384.9L125.7 354c-8.5-8.5-11.5-20.8-7.7-32.2c3-8.9 7-20.5 11.8-33.8L24 288c-8.6 0-16.6-4.6-20.9-12.1s-4.2-16.7 .2-24.1l52.5-88.5c13-21.9 36.5-35.3 61.9-35.3l82.3 0c2.4-4 4.8-7.7 7.2-11.3C289.1-4.1 411.1-8.1 483.9 5.3c11.6 2.1 20.6 11.2 22.8 22.8c13.4 72.9 9.3 194.8-111.4 276.7c-3.5 2.4-7.3 4.8-11.3 7.2v82.3c0 25.4-13.4 49-35.3 61.9l-88.5 52.5c-7.4 4.4-16.6 4.5-24.1 .2s-12.1-12.2-12.1-20.9V380.8c-14.1 4.9-26.4 8.9-35.7 11.9c-11.2 3.6-23.4 .5-31.8-7.8zM384 168a40 40 0 1 0 0-80 40 40 0 1 0 0 80z"
            />
          </svg>
          <h2 className="text-lg font-semibold">better performance</h2>
          <p className="mt-5 lg:w-3/4 sm:w-1/2 text-gray-400">
            Enjoy faster and better performance by more than 25%
          </p>
        </div>
        <div className="flex flex-col  mt-16  items-center lg:justify-start justify-center lg:w-1/4 hover:scale-105 hover:shadow-lightBlueShadow rounded-3xl transition-all p-5">
          <svg
            className="h-20 m-4"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 448 512"
          >
            <path
              fill="white"
              d="M349.4 44.6c5.9-13.7 1.5-29.7-10.6-38.5s-28.6-8-39.9 1.8l-256 224c-10 8.8-13.6 22.9-8.9 35.3S50.7 288 64 288H175.5L98.6 467.4c-5.9 13.7-1.5 29.7 10.6 38.5s28.6 8 39.9-1.8l256-224c10-8.8 13.6-22.9 8.9-35.3s-16.6-20.7-30-20.7H272.5L349.4 44.6z"
            />
          </svg>
          <h2 className="text-lg font-semibold">Lighter than light</h2>
          <p className="mt-5 lg:w-3/4 sm:w-1/2 text-gray-400">
           Only 3.4 gb of storage with no bloatwares
          </p>
        </div>
        <div className="flex flex-col  mt-16 items-center lg:justify-start justify-center lg:w-1/4 hover:scale-105 hover:shadow-lightBlueShadow rounded-3xl transition-all p-5">
          <svg
            className="h-20 m-4"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 448 512"
          >
            <path
              fill="white"
              d="M144 144v48H304V144c0-44.2-35.8-80-80-80s-80 35.8-80 80zM80 192V144C80 64.5 144.5 0 224 0s144 64.5 144 144v48h16c35.3 0 64 28.7 64 64V448c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V256c0-35.3 28.7-64 64-64H80z"
            />
          </svg>
          <h2 className="text-lg font-semibold">Privacy in chloride</h2>
          <p className="mt-5 lg:w-3/4 sm:w-1/2 text-gray-400">
            Completely get rid of Windows defender and enjoy your pc with no bloatwares{" "}
          </p>
        </div>
        <div className="flex flex-col mt-16  items-center lg:justify-start justify-center lg:w-1/4 hover:scale-105 hover:shadow-lightBlueShadow rounded-3xl transition-all p-5">
          <svg
            className="h-20 m-4"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 640 512"
          >
            <path
              fill="white"
              d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512H322.8c-3.1-8.8-3.7-18.4-1.4-27.8l15-60.1c2.8-11.3 8.6-21.5 16.8-29.7l40.3-40.3c-32.1-31-75.7-50.1-123.9-50.1H178.3zm435.5-68.3c-15.6-15.6-40.9-15.6-56.6 0l-29.4 29.4 71 71 29.4-29.4c15.6-15.6 15.6-40.9 0-56.6l-14.4-14.4zM375.9 417c-4.1 4.1-7 9.2-8.4 14.9l-15 60.1c-1.4 5.5 .2 11.2 4.2 15.2s9.7 5.6 15.2 4.2l60.1-15c5.6-1.4 10.8-4.3 14.9-8.4L576.1 358.7l-71-71L375.9 417z"
            />
          </svg>
          <h2 className="text-lg font-semibold">Personalize your device</h2>
          <p className="mt-5 lg:w-3/4 sm:w-1/2 text-gray-400">
            Get more than 50 tools and 100 commands to fully customize
            your device
          </p>
        </div>
        
        {/* background gradient */}
        <div
          className="absolute -right-0 top-0 -mt-40 -z-10 blur-3xl xl:-top-6"
          aria-hidden="true"
        >
          <div
            className="aspect-[955/1078] w-[60rem] bg-gradient-to-tl from-[#e8ff80] to-[#ee5487] opacity-40 blur-3xl"
            style={{
              clipPath:
                "polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)",
            }}
          />
        </div>
      </div>
    </>
  );
}

export default Features;
